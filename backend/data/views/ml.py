    ...
    try:
        ...
        for p in perfs:
            classification.append({
                ...
                'performance_category': p.categorie_risque,
            })
            
        if perfs.exists():
            stats_data = {
                'average_score': perfs.aggregate(Avg('moyenne_generale'))['moyenne_generale__avg'] or 0,
                'at_risk_count': perfs.filter(categorie_risque='À risque').count(),
                'good_performers': perfs.filter(categorie_risque='Bon performeur').count(),
